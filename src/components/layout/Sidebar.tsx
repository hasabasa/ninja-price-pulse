      ...
      {menuItems.map((item) => (
        <li key={item.path}>
          <Button
            ...
            className={cn(
              "w-full flex items-center justify-start px-4 py-3 text-sm rounded-lg transition-colors gap-3", 
              "items-center", // Ensure vertical centering
              isActive(item.path)
          ...
          >
            {React.cloneElement(item.icon, { 
              className: "h-5 w-5 flex-shrink-0 mx-auto" // Center icon horizontally
            })}
            {!collapsed && (
              <motion.span
                initial={{ opacity: 0 }}
